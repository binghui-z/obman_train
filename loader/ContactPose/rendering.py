...
from loader.ContactPose import misc
import cv2
...
class DepthRenderer(object):
  ...
  def __init__(self, object_name_or_mesh, K, camera_name, mesh_scale=1.0):
    ...
    if isinstance(object_name_or_mesh, str):
      filename = osp.join('../../HOinter_data/ContactPose', 'object_models',
  ...
  def object_visibility_and_projections(self, object_pose, depth_thresh=5e-3):
    ...
    cX = misc.tform_points(object_pose, self.oX)
    P = misc.get_A(self.camera_name) @ self.K @ np.eye(4)[:3]
    cx = misc.project(P, cX)
