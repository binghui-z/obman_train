...
def calc_l2_err(a, b, axis=2):
    if torch.is_tensor(a):
        ...
        mse = np.linalg.norm(a - b, 2, axis=axis)  #二范数,按列求二范数（778*3）->(778*1)
        return mse.mean()
...
def save_obj(v, f=None,file_name='output.obj'):
    """
    v: verts
    f: face of hand and object
    file_name: output path of .obj
    """
...
def render_mesh(img, mesh, face, camera_param):
    """
    img : original image
    mesh : hand verts from MANO
    face : hand face from MANO model,which is static
    camera_param : camera param ,include (fx,fy) and (cx,cy)
    """
    # mesh
    ...
    camera = pyrender.IntrinsicsCamera(fx=camera_param[0][0], fy=camera_param[1][1], 
                                       cx=camera_param[0][2], cy=camera_param[2][1])
    scene.add(camera)
    ...
    valid_mask[valid_mask[:,:,0]>0] = 255 

    # save to image
    img = rgb * valid_mask + img * (1-valid_mask)
    return rgb,depth,valid_mask[:,:,0],img
