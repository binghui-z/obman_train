...
def get_object_names(p_num, intent, ignore_hp=True):
  ...
  sess_dir = osp.join('../../', 'HOinter_data/ContactPose', 'contactpose_data', sess_dir)
...
def get_intents(p_num, object_name):
  ...
  for ins in ('use', 'handoff'):
    ...
    sess_dir = osp.join('../../', 'HOinter_data/ContactPose', 'contactpose_data', sess_dir, object_name)
...
class ContactPose(object):
  ...
  def __init__(self, p_num, intent, object_name, mano_pose_params=15,
    ...
    self.data_dir = osp.join('../../', 'HOinter_data/ContactPose',  'contactpose_data',p_id, object_name)
    ...
    for camera_name in self.valid_cameras:
      self.data_tmp_dir = self.data_dir.replace("contactpose_data","images")
      im_dir = osp.join(self.data_tmp_dir, 'images_full', camera_name, 'color')  
    ...
    oM = np.loadtxt(osp.join('../../', 'HOinter_data/ContactPose',
